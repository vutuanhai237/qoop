...
def rank_selection(self, fitness, num_parents):
    ...
    rank = np.arange(1, self.sol_per_pop+1)
...
def wheel_cumulative_probs(self, probs, num_parents):
    ...
    probs_start = np.zeros(probs.shape, dtype=float) # An array holding the start values of the ranges of probabilities.
    probs_end = np.zeros(probs.shape, dtype=float) # An array holding the end values of the ranges of probabilities.
...
def stochastic_universal_selection(self, fitness, num_parents):
    ...
    first_pointer = np.random.uniform(low=0.0, 
...
def tournament_selection_nsga2(self,
    ...
    if type(fitness[0]) in [list, tuple, np.ndarray]:
        pass
    ...
    rand_indices = np.random.randint(low=0.0, 
    ...
    # Make sure the parents indices is returned as a np array.
    return parents, np.array(parents_indices)
